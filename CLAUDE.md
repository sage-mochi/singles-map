...
- **Race**: `RAC1P` + `HISP`. **Live map** (published tables) uses overlapping "alone" definitions. **PUMS rebuild** (`build_pums_bulk.py`) uses **mutually-exclusive**: Hispanic first, then non-Hispanic White/Black/Asian/Two+/Other. Reconcile before wiring M3 to the map (open decision #2).
...
- `docs/pums-milestone1-results.md`, `docs/pums-milestone2-results.md`, `docs/pums-milestone3-results.md` — validation records.
